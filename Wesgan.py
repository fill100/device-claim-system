...
def run_asset_page():
    ...
    # ย้ายตัวกรองมาแสดงที่หน้าหลักเพื่อความเรียบร้อย
    all_models = ["ทั้งหมด"] + sorted(df["Model Name (ชื่อรุ่น)"].unique().tolist())
    filter_model = st.selectbox("🎯 เลือกดูเฉพาะรุ่น (ตัวกรอง Model):", all_models, key="asset_filter_selectbox_sp_fixed")
    ...
    with st.expander(expander_label, expanded=is_editing):
        with st.form("asset_form_sp_fixed", clear_on_submit=True):
            current_val = st.session_state.edit_data if is_editing else {}
            
            col1, col2 = st.columns(2)
            with col1:
                input_sn = st.text_input("Serial Number", value=current_val.get("Serial Number (เลขซีเรียล)", ""), key="asset_sn_input_fixed")
                input_model = st.text_input("Model Name", value=current_val.get("Model Name (ชื่อรุ่น)", ""), key="asset_model_input_fixed")
            with col2:
                input_loc = st.text_input("Location", value=current_val.get("Location (สถานที่)", ""), key="asset_loc_input_fixed")
                ...
                input_date = st.date_input("วันที่ซื้อ", value=default_date, format="DD/MM/YYYY", key="asset_date_input_fixed")
    ...
    with c1:
        search_term = st.text_input("🔍 ค้นหาในตาราง (S/N, รุ่น, สถานที่):", key="asset_search_fixed")
        if search_term:
    ...
    edited_view_df = st.data_editor(
        ...
        key="bulk_edit_location_fixed"
    )

    if st.button("✅ ยืนยันการเปลี่ยนสถานที่ในตาราง", key="btn_confirm_asset_loc_fixed"):
        try:
