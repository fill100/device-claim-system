...
# --- ฟังก์ชันสำหรับโหลดไฟล์ภายนอก (ดึงระบบอื่นมารันร่วมอย่างสมบูรณ์) ---
def show_asset_system(conn):
    try:
        ...
        st.error(f"เกิดข้อผิดพลาดในการเปิดระบบ Asset System: {e}")
...
def show_transfer_system(conn):
    try:
        ...
        st.error(f"⚠️ เกิดข้อผิดพลาดในการเปิดระบบโอนย้าย: {e}")

# --- ตั้งค่าหน้ากระดาษ ---
st.set_page_config(page_title="💻 JVFS IT Management System", layout="wide")

# --- ระบบจัดเก็บสถานะหน้าปัจจุบันสำหรับปุ่ม Sidebar ---
...
st.markdown("""
    ...
    /* ซ่อนเมนูนำทางเริ่มต้นของระบบออกทั้งหมด */
...
# --- 3. Sidebar Menu (แก้ไขเพื่อแก้ปัญหา page_link เออเร่อลูปหน้าตัวเอง) ---
with st.sidebar:
    st.markdown("# 💻 IT Management")
    
    # ใช้ปุ่มสลับหน้าที่สอดคล้องกับสีเมนูเดิมอย่างสมเหตุสมผล
    ...
    # เมนูการตั้งค่าเหล่านี้จะทำงานเฉพาะเมื่ออยู่ในหน้าหลัก Device Claim เท่านั้นเพื่อไม่ให้ทับซ้อนระบบอื่น
...
# --- ระบบ Routing ควบคุมการทำงานของหน้าหลัก ---
if st.session_state.current_page == "Asset System":
    show_asset_system(conn)
    st.stop()  # หยุดรันส่วนล่างเพื่อส่งพื้นที่ให้ไฟล์ภายนอกทำอย่างสมบูรณ์
