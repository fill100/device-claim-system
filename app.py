...
# --- ส่วนที่ 2: ฟอร์มบันทึกข้อมูล (ตัดช่องสถานะออก) ---
with st.expander("➕ เพิ่มข้อมูลการเคลมใหม่"):
    with st.form("main_form", clear_on_submit=True):
        ...
        with f2:
            ...
            # เพิ่มช่องวันที่เพื่อให้ข้อมูลครบถ้วน
...
with s_col:
    q = st.text_input("ค้นหา:", placeholder="พิมพ์ Serial, สาขา หรือ Counter...")
with r_col:
...
# ส่วนแก้ไข (ตัดการอัปเดตสถานะออก)
if not view_df.empty:
    with st.expander("📝 แก้ไขข้อมูล Serial"):
        sn_list = view_df["Serial เครื่องที่เสีย"].loc[view_df["Serial เครื่องที่เสีย"] != ""].unique().tolist()
        if sn_list:
            target_sn = st.selectbox("เลือกเครื่องที่ต้องการแก้ไข:", sn_list)
            row_data = df[df["Serial เครื่องที่เสีย"] == target_sn].iloc[0]
            
            with st.form("quick_edit"):
                e1, e2 = st.columns(2)
                with e1:
                    new_sn_center = st.text_input("S/N ส่งศูนย์", value=str(row_data["Serial เครื่องที่ส่งให้ศูนย์"]))
                with e2:
                    new_sn_new = st.text_input("S/N เครื่องใหม่", value=str(row_data["Serial เครื่องที่เปลี่ยนใหม่"]))
